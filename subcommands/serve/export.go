...
func (s *Server) Export(ctx context.Context, r *connect.Request[earbugv4.ExportRequest]) (*connect.Response[earbugv4.ExportResponse], error) {
	...
	if err != nil {
		return nil, s.o.Err(ctx, "marshal store", err)
	}
	...
	if r.Msg.Bucket == "" {
		...
		if err != nil {
			return nil, s.o.Err(ctx, "open destination bucket", err)
		}

		ow, err := bkt.NewWriter(ctx, r.Msg.Key, nil)
		if err != nil {
			return nil, s.o.Err(ctx, "open destination key", err)
		}
		defer ow.Close()
		zw, err := zstd.NewWriter(ow)
		if err != nil {
			return nil, s.o.Err(ctx, "new zstd writer", err)
		}
		defer zw.Close()
		_, err = io.Copy(zw, bytes.NewReader(b))
		if err != nil {
			return nil, s.o.Err(ctx, "write store", err)
		}
	}
	return res, nil
}
